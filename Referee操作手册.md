...
**！本文档按键功能说明仅通过代码分析写出，未完全经过实车测试，本文档将会持续更新**



##  可操作按键示意图

![https://i0.hdslb.com/bfs/album/225655a0bd5a8657da8c6564c487fd3290242534.jpg@1036w.webp](https://i0.hdslb.com/bfs/album/225655a0bd5a8657da8c6564c487fd3290242534.jpg@1036w.webp)
...
| ctrl+v | 校准敌方颜色                 |
| ctrl+b | 调整相机曝光度               |
| ctrl+r | 设置为锁定模式，底盘不再移动 |
| shift  | 开启超级电容                 |




| 拨动开关 | Gimbal        | PowerLimit     | ShoorCmd              | Status           |
| -------- | ------------- | -------------- | --------------------- | ---------------- |
| 右 + 上  | RATE （跟随） | CHARGE（充电） | STOP（停止射击）      | PC（键鼠控制）   |
| 右 + 中  | RARE （跟随） | CHARGE（充电） | STOP（停止射击）      | RC（遥控器模式） |
| 右 + 下  | RATE （跟随） | CHARGE（充电） | STOP（停止射击）      | IDLE             |
| 左 + 上  | TRACK（自瞄） |                | PUSH  （自瞄并射击）  | 自动操作并射击   |
...
| 拨动开关 | Gimbal        | PowerLimit     | ShoorCmd              | Status           | 舱门指令 |
| -------- | ------------- | -------------- | --------------------- | ---------------- | -------- |
| 右 + 上  | RATE （跟随） | CHARGE（充电） | STOP（停止射击）      | PC（键鼠控制）   | 关闭     |
| 右 + 中  | RARE （跟随） | CHARGE（充电） | STOP（停止射击）      | RC（遥控器模式） | 关闭     |
| 右 + 下  | RATE （跟随） | CHARGE（充电） | STOP（停止射击）      | IDLE             | 开启     |
| 左 + 上  | TRACK（自瞄） |                | PUSH  （自瞄并射击）  | 自动操作并射击   |          |
| 左 + 中  | TRACK（自瞄） |                | READY（仅自瞄不射击） | 自动操作不射击   |          |
| 左 + 下  | RATE （跟随） |                | STOP（停止射击）      | 手动操作射击     |          |
